...
from ultralytics.yolo.data.utils import check_cls_dataset, check_det_dataset, check_multi_dataset
...
class BaseTrainer:
    ...
    def _setup_train(self, world_size):
        ...
        if RANK in (-1, 0):
            if self.args.task == "multi":
                ...
                self.validator = self.get_validator() #get_validator() call in def get_validator() in train.py
    ...
    def _do_train(self, world_size=1):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            self.model.train()
            """
                Nếu RANK != -1, có nghĩa là mô hình đang huấn luyện phân tán, 
                và set_epoch đảm bảo dữ liệu được lấy mẫu (sampling) ngẫu nhiên cho mỗi epoch.
            """
            if RANK != -1:
            ...
            """
                Khi đến một epoch nhất định (xác định bởi close_mosaic), 
                hàm này tắt chế độ mosaic (một kỹ thuật để kết hợp nhiều hình ảnh nhỏ lại với nhau, 
                giúp mô hình có thêm nhiều biến thể dữ liệu).
            ...
            for i, batch in pbar:
                ...
                # Warmup
            """
                Giai đoạn "warmup" điều chỉnh các tham số như learning rate (tốc độ học) và 
                momentum dần dần để tránh việc mô hình học quá nhanh và gây ra mất ổn định.
                np.interp sử dụng để nội suy các giá trị của tốc độ học và 
                momentum từ một giá trị thấp đến giá trị mong muốn trong quá trình warmup.
            """
                ni = i + nb * epoch
                if ni <= nw:
                    xi = [0, nw]  # x interp
                ...
                """
                with torch.cuda.amp.autocast(self.amp): #self.amp: Biến này chỉ định xem mô hình có đang chạy với chế độ mixed precision hay không.
