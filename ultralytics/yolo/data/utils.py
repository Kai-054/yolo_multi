...
def verify_image_label(args):
    ...
    try:
        ...
        # Verify labels
        if os.path.isfile(lb_file):  # lb_file: Đường dẫn đến file nhãn.
            nf = 1  # label found
            with open(lb_file) as f:
                lb = [x.split() for x in f.read().strip().splitlines() if len(x)]
                if any(len(x) > 7 for x in lb) and (not keypoint):  # is segment
                    classes = np.array([x[:2] for x in lb], dtype=np.float32)
                    segments = [np.array(x[2:], dtype=np.float32).reshape(-1, 2) for x in lb]  # (cls, xy1...)
                    lb = np.concatenate((classes, segments2boxes(segments)), 1)  # (cls1, cls2, xywh)
                lb = np.array(lb, dtype=np.float32)
            nl = len(lb)
            if nl:
                if keypoint:
